...
function EventDetailsPage() {
  ...
  return (
    <main className="event-detail-page">
      ...
      {notice ? (
        <div className="event-detail-trip-toast">
          {notice}
        </div>
      ) : null}
      ...
      <section className="event-hero">
        ...
        <div className="event-hero-copy">
          ...
          <h1>{event.title}</h1>
          <p>
            📍 {event.venue}, {event.city} · {event.dateLabel} · {event.priceLabel}
          </p>
...
.not-found-card{width:min(720px,calc(100% - 32px));margin:80px auto;padding:42px;text-align:center;background:#fff;border:1px solid #e0e9e5;border-radius:24px;box-shadow:0 18px 50px rgba(15,58,50,.08)}.not-found-card>span{display:block;font-size:42px}.not-found-card h1{margin:14px 0 8px;color:#123f3a}.not-found-card p{color:#66756f;line-height:1.6}.not-found-card a{display:inline-flex;margin-top:14px;padding:11px 16px;border-radius:10px;background:#0f766e;color:#fff;text-decoration:none;font-weight:800}.event-loading-card{min-height:260px;display:grid;place-items:center;align-content:center}
`;
