...
function PartnerGuideRegistrationPage() {
  ...
  const payRegistrationFee = async (guide) => {
    setPaymentRequest({
      type: "registration",
      guide,
      title: "Guide registration fee",
      description: "Pay the guide registration fee before admin approval.",
      amount: guide.registration_fee,
      submitLabel: "Pay registration fee",
    });
  };

  const payPromotionFee = async (guide) => {
    setPaymentRequest({
      type: "promotion",
      guide,
      title: "Guide top-ad promotion",
      description: "Pay the optional top listing fee to keep this approved guide near the top.",
      amount: guide.promotion_fee,
      submitLabel: "Pay top-ad fee",
    });
  };

  const confirmGuidePayment = async ({ gateway, card_last4 }) => {
    if (!paymentRequest?.guide) return;
    ...
    try {
      ...
      if (paymentRequest.type === "registration") {
        await api.post(`/partner/guides/${paymentRequest.guide.id}/pay-registration`, {
          payment_gateway: gateway,
          card_last4,
        });
        setMessage("Guide registration fee paid successfully. Admin can now approve this guide.");
      } else {
        await api.post(`/partner/guides/${paymentRequest.guide.id}/pay-promotion`, {
          payment_gateway: gateway,
          card_last4,
        });
        setMessage("Top-listing promotion paid successfully. Approved promoted guides appear before normal guides.");
      }

      setPaymentRequest(null);
      await loadGuides();
    } catch (err) {
      setError(err.response?.data?.message || "Failed to complete guide payment");
      throw err;
    }
  };
  ...
  return (
    <main className="partner-guide-page">
      ...
      <PaymentModal
        ...
        reference={paymentRequest?.guide?.display_name || ""}
        submitLabel={paymentRequest?.submitLabel || "Pay now"}
        onClose={() => setPaymentRequest(null)}
        onConfirm={confirmGuidePayment}
      />
      ...
      <section className="guide-form-hero">
        <div>
          ...
          <p>
            Add your guide profile, languages, services, pricing, and travel expertise. Pay the registration
            fee before admin approval, then optionally promote your approved ad near the top.
          </p>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="guide-payment-note">
                <strong>Payment process:</strong> after saving the profile, use the payment buttons in your
                profile card. Admin approval is available only after the registration fee is paid.
              </div>
            </>
          )}
        </form>
                ...
                <article key={guide.id} className="my-guide-card">
                  ...
                  <div>
                    ...
                    <div className="guide-payment-mini">
                      <span className={guide.registration_payment_status === "Paid" ? "paid" : "unpaid"}>
                        Registration: {guide.registration_payment_status || "Unpaid"} ({formatMoney(guide.registration_fee)})
                      </span>
                      <span className={guide.is_promoted ? "paid" : "unpaid"}>
                        Top ad: {guide.promotion_payment_status || "Unpaid"}
                        {guide.is_promoted ? ` until ${formatDate(guide.promotion_expires_at)}` : ""}
                      </span>
                    </div>
                    ...
                    <div className="mini-guide-actions">
                      ...
                      {guide.status === "approved" && guide.registration_payment_status === "Paid" && (
                        <Link className="requests-mini" to={`/partner/guides/${guide.id}/requests`}>
                          Manage requests
                        </Link>
                      )}
                      {guide.registration_payment_status !== "Paid" && (
                        <button type="button" className="pay-mini" onClick={() => payRegistrationFee(guide)}>
                          Pay registration
                        </button>
                      )}
                      {guide.registration_payment_status === "Paid" && guide.status === "approved" && !guide.is_promoted && (
                        <button type="button" className="promote-mini" onClick={() => payPromotionFee(guide)}>
                          Promote top ad
                        </button>
                      )}
                      {guide.registration_payment_status === "Paid" && guide.status !== "approved" && !guide.is_promoted && (
                        <span className="promotion-waiting">Promotion available after approval</span>
                      )}
...
.guide-form-hero:before {
  ...
  inset: 0 0 auto;
  height: 4px;
  background: linear-gradient(90deg,#08786d 0%,#11a08e 43%,#d6a624 100%);
}
...
.guide-stat-card {
  ...
  border-radius: 12px;
  padding: 18px;
  box-shadow: 0 8px 24px rgba(22,53,46,.035);
}
...
.my-guides-panel {
  ...
  border-radius: 16px;
  box-shadow: 0 10px 30px rgba(22,53,46,.045);
}
...
.submit-guide-btn {
  ...
  font-weight: 800;
  cursor: pointer;
  box-shadow: 0 10px 24px rgba(15,130,118,.16);
}
